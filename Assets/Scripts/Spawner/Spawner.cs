...
public class Spawner : MonoBehaviour {
    ...
    private void GenerateFullLine(SpawnPoint[] spawnPoints, GameObject spawnObject) {
        for (var i = 0; i < spawnPoints.Length; i++) GenerateElement(spawnPoints[i].transform.position, spawnObject);
    }

    private void GenerateRandomLine(SpawnPoint[] spawnPoints, GameObject spawnObject, int spawnChance) {
        for (var i = 0; i < spawnPoints.Length; i++) {
            if (Random.Range(0, 100) < spawnChance) GenerateElement(spawnPoints[i].transform.position, spawnObject);
        }
    }
